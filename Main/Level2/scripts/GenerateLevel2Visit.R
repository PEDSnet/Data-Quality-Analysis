...
generateLevel2Visit <- function () {
  ...
  message("temp outliers check complete")
  ...
  message("incondatetime start check complete")
  log_entry_content<-(read.csv(log_file_name))
  ...
  message("incondatetime end check complete")
  ...
  message("Pre birth check complete")
  ...
  write.csv(log_entry_content, file = log_file_name,row.names=FALSE)
  message("post death check complete")
  log_entry_content<-(read.csv(log_file_name))
  log_entry_content<-custom_rbind(log_entry_content,applyCheck(MissVisitFact(), c(table_name), NULL)) 
  write.csv(log_entry_content, file = log_file_name ,row.names=FALSE)

  message("miss visit fact check complete")
  ### inpatient visits with no DRG data 
  log_entry_content<-(read.csv(log_file_name))
  try(log_entry_content<-custom_rbind(log_entry_content,applyCheck(MissVisitTypeFact(), c(table_name, 'observation'),
                                                                 c('9201','observation_concept_id','3040464'),
                                                               "inpatient visits with no DRG data"
                                                               )), silent = F)
  write.csv(log_entry_content, file = log_file_name,row.names=FALSE)
  message("miss visit fact type 1 check complete")

  ### inpatient visits with 
  log_entry_content<-(read.csv(log_file_name))
  try(log_entry_content<-custom_rbind(log_entry_content,applyCheck(MissVisitTypeFact(), c(table_name, 'visit_payer'),
  ...
  message("miss visit fact type 2 check complete")
