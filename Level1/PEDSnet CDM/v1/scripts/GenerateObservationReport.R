...
concept_id_list <- unique(df_table$observation_concept_id)
...
{
  df_table_subset<- df_table[df_table$observation_concept_id==concept_id_list[i],]
  field_name_subset<-paste(field_name,concept_id_list[i],sep="_")
  colnames(df_table_subset)[column_index] <- field_name_subset
  fileContent <-c(fileContent,paste("## Barplot for",field_name_subset,"\n"))
